...
public class TProductServiceImpl extends ServiceImpl<TProductMapper, TProduct>
    ...
    public Result queryByKeyWord(Integer pageNum, Integer pageSize, String keyword) throws IOException {
        ...
        if (keyword.length() == 0) {
            ...
            MultiMatchQueryBuilder multiMatchQueryBuilder = QueryBuilders.multiMatchQuery(keyword).field("name").field("detail").field("categoryName");
            boolQueryBuilder.must(QueryBuilders.boolQuery().must(multiMatchQueryBuilder)).
                    must(QueryBuilders.termQuery("status", 1)).
                    must(QueryBuilders.termQuery("deleted", 1));
        }
